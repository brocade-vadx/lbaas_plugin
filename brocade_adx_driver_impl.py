...
from neutron.services.loadbalancer.drivers.brocade_vlb import driver as vlb_drv
...
class BrocadeAdxDeviceDriverImpl():
    def __init__(self, user, secret, ip):

        client = vlb_drv.AdxService(ip, user, secret)
        slbServiceClient = client.createSlbServiceClient()
        netServiceClient = client.createNetServiceClient()

        self.slb_factory = slbServiceClient.factory
        self.slb_service = slbServiceClient.service

        self.sys_service_client = client.createSysServiceClient()

        self.net_factory = netServiceClient.factory
        self.net_service = netServiceClient.service

    def _get_pool(self, pool_id):
        session = db.get_session()
        query = session.query(lb_db.Pool)
        pool = query.filter(lb_db.Pool.id == pool_id).one()
        return pool

    def _get_vip(self, vip_id):
        session = db.get_session()
        query = session.query(lb_db.Vip)
        vip = query.filter(lb_db.Vip.id == vip_id).one()
        return vip

    def _get_pool_members(self, pool_id):
        session = db.get_session()
        query = session.query(lb_db.Member)
        pool = query.filter(lb_db.Pool.id == pool_id).all()
        return pool

    def _get_health_monitor(self, pool_id):
        ''' TODO: validate more than 1 health association '''
        session = db.get_session()
        query = session.query(lb_db.PoolMonitorAssociation)
        hma = query.filter(lb_db.PoolMonitorAssociation.pool_id ==
                pool_id).all()
        if len(hma) > 1:
            ...
            m = _('Health monitor for pool %s not found' % pool_id)
            LOG.error(m)
    ...
    def create_health_monitor(self, health_monitor, pool_id):
        ...
        try:
            health_monitor = self._get_health_monitor(pool_id)
